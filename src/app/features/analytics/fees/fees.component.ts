...
@Component({
  selector: 'app-fees',
  standalone: true,
  imports: [DsTableToolbarComponent, FeesFilterShellComponent, SaveViewModalComponent],
...
export class FeesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @ViewChild(FeesFilterShellComponent) private filterShell!: FeesFilterShellComponent;
  ...
  onSaveViewConfirmed(name: string): void {
    ...
    if (this.isSavedView && this.currentSavedView) {
      ...
      this.router.navigate(['/analytics/fees/saved-views', newView.id]);
    }
  }
  ...
  onSaveViewDeleted(): void {
    ...
    this.router.navigate(['/analytics/fees']);
  }
